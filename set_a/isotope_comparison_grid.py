...
use_tex()
fig, axes = plt.subplots(1, 3, figsize=(10, 4), dpi=600, sharex=True, sharey=True)
...
for ax, rc in zip(axes, rc_values):
  ...
  for i_m, interloper_mass in enumerate(interloper_masses):
    for i_v, interloper_velocity in enumerate(interloper_velocities):
      ...
      ax.scatter(ddmx, ddmy, c=[colours[i_v]], marker=markers[i_m], s=6, linewidths=0, label=label, zorder=2)
...
plt.savefig("isotope_comparison_grid.pdf".format(rc),bbox_inches="tight")