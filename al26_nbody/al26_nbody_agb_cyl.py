"""
26al_nbody_cyl.py
- This code is a rewritten version of al26_nbody.py, this only handles wind enrichment from AGB
  stars, using the cylindral method described in the paper, large sections have been excised or
  commented out!
...
interloper_positions = []
interloper_seg = []
interloper_seg_length = []
interloper_seg_mass_26al = []
interloper_seg_mass_60fe = []
...
def evolve_simulation(cluster,converter,gravity,stellar,yields,metadata,t_f,Rc,bar,save,AGB=None):
  ...
  ### CYLINDRICAL INTERLOPER ROTUINES
  if metadata.args.interloper:
    time_offset = metadata.args.interloper_offset_time
    interloper_time = t - time_offset

    if interloper_time > 0.0 | myr and interloper_time < AGB.t_f:
      # Calculate properties of interloper cylinder segments
      # Update interloper position
      interloper_positions.append(gravity.particles[-1].position)
      # Once there are enough interloper positions, calculate the other properties
      if len(interloper_positions) > 1:
        # Each segment is a tuple of (start, end) positions
        seg = (interloper_positions[-2], interloper_positions[-1])
        cyl_l = calc_star_distance(interloper_positions[-2], interloper_positions[-1])
        interloper_seg.append(seg)
        interloper_seg_length.append(cyl_l)
        # Get interloper mass loss rates
        al26_mass_loss = AGB.interp_value("26al_mass_loss_rate",interloper_time)
        fe60_mass_loss = AGB.interp_value("60fe_mass_loss_rate",interloper_time)
        interloper_seg_mass_26al.append(al26_mass_loss * dt)
        interloper_seg_mass_60fe.append(fe60_mass_loss * dt)
        # print(interloper_seg_mass_26al[-1].value_in(msol),interloper_seg_mass_60fe[-1].value_in(msol))

    # Get the other necessary parameters
    agb_cyl_radius = metadata.args.interloper_bubble_radius
    
    # Now, calculate the mass deposition for each disk-bearing star
    for i in lm_id:
      if cluster[i].disk_alive == True:
        # Get the position of the star, before and after timestep
        pos_o = gravity_prev[i].position
        pos_n = gravity.particles[i].position
        # Now, iterate over the interloper cylinders for the given star
        for j in range(len(interloper_seg)):
          agb_cyl_length = interloper_seg_length[j]
          # Calculate the fraction of the timestep the star spends inside a cylinder
          intersection_frac = line_cylinder_intersection_fraction(
            np.array(pos_o.value_in(pc)),
            np.array(pos_n.value_in(pc)),
            np.array(interloper_seg[j][0].value_in(pc)),
            np.array(interloper_seg[j][1].value_in(pc)),
            agb_cyl_radius.value_in(pc),
          )
          # If the star is inside the cylinder for any amount of time, calculate the mass deposition
          if intersection_frac != 0.0:
            disk_dist = (pos_n - pos_o).length() # Distance travelled by the star during the timestep
            r_disk = cluster[i].r_disk # Disk radius
            eta_cyl = (r_disk**2 * disk_dist) / (agb_cyl_length * agb_cyl_radius**2)
            # Calculate the mass deposition for each SLR
            agb_cyl_abs_al26 = intersection_frac * eta_cyl * interloper_seg_mass_26al[j]
            agb_cyl_abs_fe60 = intersection_frac * eta_cyl * interloper_seg_mass_60fe[j]
            # Update arrays
            cluster[i].mass_26al_agb += agb_cyl_abs_al26
            cluster[i].mass_60fe_agb += agb_cyl_abs_fe60
            cluster[i].mass_26al_agb_raw += agb_cyl_abs_al26
            cluster[i].mass_60fe_agb_raw += agb_cyl_abs_fe60
            # Also update the interloper segment mass arrays
            interloper_seg_mass_26al[j] -= agb_cyl_abs_al26
            interloper_seg_mass_60fe[j] -= agb_cyl_abs_fe60
  ...
  if metadata.args.interloper:
    ...
    # Decay material in agb cylinders
    for i in range(len(interloper_seg)):
      interloper_seg_mass_26al[i] *= decay_frac_26al
      interloper_seg_mass_60fe[i] *= decay_frac_60fe
  ...
  for i in lm_id:
    if cluster[i].disk_alive == True:
      ...
      if cluster[i].tau_disk < t_new:
        # if metadata.args.verbose:
        bar.write(
          "Disk of low-mass star #{} has condensed, final 26Al/27Al in disk: {:.3e}".format(
            i, cluster[i].mass_26al_agb_final / cluster[i].mass_27al
          )
        )
...
@nb.njit()
def line_cylinder_intersection_fraction(line_start, line_end, cyl_start, cyl_end, cyl_radius):
    """
    Calculate the fraction of the length of a line segment that intersects a cylinder.

    Args:
        line_start: (x, y, z) array for the start of the line segment.
        line_end: (x, y, z) array for the end of the line segment.
        cyl_start: (x, y, z) array for the start of the cylinder axis.
        cyl_end: (x, y, z) array for the end of the cylinder axis.
        cyl_radius: Cylinder radius (same units as coordinates).

    Returns:
        Fraction of the line segment's length that lies inside the cylinder (between 0 and 1).
    """
    ...
    # Move to cylinder frame
    dp = p0 - c0

    # Components perpendicular to cylinder axis
    d_perp = d - np.dot(d, v_norm) * v_norm
    dp_perp = dp - np.dot(dp, v_norm) * v_norm

    # Quadratic coefficients for intersection
    a = np.dot(d_perp, d_perp)
    b = 2 * np.dot(d_perp, dp_perp)
    c = np.dot(dp_perp, dp_perp) - r**2
    ...
    t_in = max(0, min(t0, t1))
    t_out = min(1, max(t0, t1))
    if t_out < t_in:
        return 0.0

    # Check if intersection points are within the finite cylinder caps
    def within_cylinder_caps(t):
        pt = p0 + t * d
        proj = np.dot(pt - c0, v_norm)
        return 0 <= proj <= np.linalg.norm(v)

    # Find valid interval inside both the infinite cylinder and the finite caps
    ts = [t for t in [t_in, t_out] if 0 <= t <= 1 and within_cylinder_caps(t)]
    if len(ts) < 2:
        # Try to find overlap with caps
        cap0 = np.dot(p0 - c0, v_norm)
        cap1 = np.dot(p1 - c0, v_norm)
        cap_min = min(cap0, cap1)
        cap_max = max(cap0, cap1)
        cyl_len = np.linalg.norm(v)
        if cap_max < 0 or cap_min > cyl_len:
            return 0.0
        # Clamp to cylinder caps
        t_cap_in = (0 - cap0) / (cap1 - cap0) if cap1 != cap0 else 0
        t_cap_out = (cyl_len - cap0) / (cap1 - cap0) if cap1 != cap0 else 1
        t_in = max(0, min(t_in, t_out, t_cap_in, t_cap_out))
        t_out = min(1, max(t_in, t_out, t_cap_in, t_cap_out))
        if t_out < t_in:
            return 0.0
        ts = [t_in, t_out]

    fraction = max(0.0, ts[1] - ts[0])
    return fraction
