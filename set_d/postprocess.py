...
proc_data["interloper_bubble_radius"] = []
# Star properties
proc_data["star"] = []
...
for simset in tt1:
  ...
  for sim_number,sim in enumerate(tt2):
    ...
    if has_interloper:
      ...
      interloper_bubble_radius = metadata.args.interloper_bubble_radius.value_in(units.pc)
    else:
      interloper_mass = 0.0
      interloper_velocity = 0.0
      interloper_time_offset = 0.0
      interloper_radius_offset = 0.0
      interloper_bubble_radius = 0.0
    ...
    for star_number,star in enumerate(cluster):
      ...
      for iso_index,isotope in enumerate(isotopes):
        for model in models:
          ...
          proc_data["interloper_bubble_radius"].append(interloper_bubble_radius)
          # Star properties
          ...
          proc_data["yield_raw"].append(unstable_yield_raw.value_in(msol))
